...
export async function GET(request: NextRequest) {
    ...
    console.log('🔍 Preview request received:', { token: !!token, category, id, slug });

    // Validate required parameters
    if (!token || (!id && !slug)) {
        console.error('❌ Missing required parameters');
        return NextResponse.json(
            { error: 'Missing token, id, or slug parameter' },
            { status: 400 }
        );
    }
    ...
    try {
        ...
        if (!validateResponse.ok) {
            let msg = 'Invalid or expired preview token';
            try {
        ...
        // 3. Construct the preview URL using the provided category and id or slug
        // Use the article route pattern: /[category]/[id] or falling back to slug
        let previewPath = '';
        if (slug) {
            previewPath = slug;
        } else {
            previewPath = category && category !== 'uncategorized' ? `/${category}/${id}` : `/news/${id}`;
        }
