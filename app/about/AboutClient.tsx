...
export default function AboutClient({ data }: { data: AboutPageData }) {
  ...
  const findSection = <T extends AboutPageData["sections"][number]>(
    type: T["fieldGroupName"],
  ) => {
    return sections.find((s) => s.fieldGroupName === type) as T | undefined;
  };

  const heroSection = findSection<AboutPageHeroSection>(
    "AboutPageBuilderAboutPageBuilderHeroSectionLayout",
  );
  const visionSection = findSection<AboutPageVisionMessageSection>(
    "AboutPageBuilderAboutPageBuilderVisionMessageSectionLayout",
  );
  const coreValuesSection = findSection<AboutPageCoreValuesSection>(
    "AboutPageBuilderAboutPageBuilderCoreValuesSectionLayout",
  );
  const boardSection = findSection<AboutPageBoardOfDirectorsSection>(
    "AboutPageBuilderAboutPageBuilderBoardOfDirectorsSectionLayout",
  );
  const historySection = findSection<AboutPageClubHistorySection>(
    "AboutPageBuilderAboutPageBuilderClubHistorySectionLayout",
  );

  // Board members layout requirement:
  // - first 2 directors show in the first row
  // - remaining directors show in the second row
  const firstTwoDirectors = boardSection?.directors?.slice(0, 2) ?? [];
  ...
  return (
    <div className="pt-25 pb-25">
      ...
      <div className="container max-w-7xl mx-auto px-4 md:px-6">
              ...
              <div className="md:w-1/2 p-8 md:p-12 text-right flex flex-col justify-center gap-4 bg-white">
                {heroSection.preTitle && (
                  <div className="inline-flex items-center gap-3 justify-end mb-2">
                    ...
                    <span className="text-sm font-bold text-club-purple">
                      {heroSection.preTitle}
                    </span>
                  </div>
                )}
            ...
            <section className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-24">
              {visionSection.cards.map((card, index) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-8 ${index === 0 ? "bg-club-blue/10 text-club-blue" : "bg-club-purple/10 text-club-purple"}`}
                    >
        ...
        {/* Core Values */}
        {coreValuesSection &&
          coreValuesSection.values &&
          coreValuesSection.values.length > 0 && (
            <section className="mb-24">
                ...
                {coreValuesSection.values.map((value, i) => {
                  ...
                  if (value.icon && fallbackMap[value.icon.toLowerCase()]) {
                    ...
                    const formattedIconName =
                      value.icon.charAt(0).toUpperCase() + value.icon.slice(1);
                  ...
                  return (
                    ...
                    >
                      <div className="text-club-purple mb-6">
        ...
        {/* Board Members */}
        {boardSection &&
          boardSection.directors &&
          boardSection.directors.length > 0 && (
            <section className="mb-24">
              <div className="text-center mb-12">
              ...
              <div className="flex flex-col sm:flex-row justify-center mb-16 items-center gap-30 mt-16">
                {firstTwoDirectors.map((member, i) => (
                  ...
                  >
                    ...
                    {member.role && (
                      <p className="text-club-purple font-medium text-sm">
                        {member.role}
                      </p>
                    )}
                  </div>
        ...
        {historySection &&
          ...
          historySection.historyItems.length > 0 && (
            <section className="mb-24">
              ...
              <div className="bg-white rounded-[2rem] shadow-lg border border-border p-6 md:p-8">
                <HistorySlider
                  historyItems={historySection.historyItems}
                  footerText={historySection.footerText}
                />
              </div>
            </section>
          )}
...
const HistorySlider = ({
  ...
  const visibleMilestones = React.useMemo(
    () =>
      Array.from(
        { length: itemsToShow },
        (_, k) => historyItems[(index + k) % historyItems.length],
      ),
  ...
  return (
    <div className="relative max-w-6xl mx-auto">
      ...
      >
        <div
          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-${itemsToShow} gap-6`}
        >
          {visibleMilestones.map((milestone, i) => (
            ...
            >
              <div className="relative h-40">
                <ImageWithFallback
                  src={normalizeImageUrl(
                    milestone.image?.node?.sourceUrl || "",
                  )}
