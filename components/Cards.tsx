  ...
  >
    ...
    <div className="p-6 flex flex-col grow">
      <Link href={`/events/${event.id}`}>
        <h3 className="text-xl font-bold mb-4 group-hover:text-club-purple transition-colors line-clamp-2 min-h-14">
          {event.title}
        </h3>
      </Link>
...
export const NewsCard = ({ news }: { news: any }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-card rounded-2xl overflow-hidden border border-border group flex flex-col h-full shadow-sm hover:shadow-md transition-all"
    >
      <Link href={postUrl} className="relative h-60 overflow-hidden block">
        <ImageWithFallback
          src={news.image}
          alt={news.title}
          className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
